...
func (voto Voto) Devolver_votos() [CANT_VOTACION]int {
	return voto.VotoPorTipo
}

func CrearVoto() Voto {
	return Voto{[CANT_VOTACION]int{}, false}
}

func (voto *Voto) Votar(cargo TipoVoto, alternativa int) {
	voto.VotoPorTipo[cargo] = alternativa
}

func (voto Voto) NumPresidente() int {
	return voto.VotoPorTipo[PRESIDENTE]
}

func (voto Voto) NumGobernador() int {
	return voto.VotoPorTipo[GOBERNADOR]
}

func (voto Voto) NumIntendente() int {
	return voto.VotoPorTipo[INTENDENTE]
}
